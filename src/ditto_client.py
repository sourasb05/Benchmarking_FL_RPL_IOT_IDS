...
class DittoClient:
    def __init__(self, client_id, args, domain_path, assigned_domains, device, model, lam):
        ...
        self.personalized_optimizer = optim.Adam(self.personalized_model.parameters(), lr=self.args.lr)  # Separate optimizer for personalized model
    ...
    def train(self, global_model_state, time_step):
        ...
        if time_step == 0:
            self.personalized_model.load_state_dict(copy.deepcopy(global_model_state))
        ...
        for epoch in range(self.args.local_epochs):
            ...
            for batch_idx, (data, target) in enumerate(self.train_domains_loader[self.domain_keys[time_step]]):
                ...
                total_loss = loss + (self.lam / 2) * proximal_term
                total_loss.backward()
