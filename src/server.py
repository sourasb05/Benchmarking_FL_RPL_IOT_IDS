...
def server(args, model, device, domains_path, client_distributions, max_client_participants, project_root):
    """
    FedAvg — single time step.

    Per global iteration:
      1. Each client trains from the current global model weights.
         The trained local model is evaluated immediately on the client's
         own test data. Best local model per client is saved.
      2. All local models are aggregated (FedAvg) into a new global model.
         The global model is evaluated across all clients.

    After all iterations:
      • Per-client convergence plots (Accuracy & F1, local vs global)
      • Load best global model  → evaluate on all clients
      • Load best local models  → evaluate on all clients
