...
def _print_comparison_table(client_list, best_global_results, best_local_results):
    """Side-by-side table: best global model vs best local model per client."""
    header = (
        f"{'Client':>8} | "
    ...
    sep = "-" * len(header)
    print(f"\n{sep}")
    print("  FINAL: Best Global Model vs Best Local Model per Client  [SCAFFOLD]")
    print(sep)
    print(header)
    print(sep)
...
def scaffold_server(args, model, device, domains_path, client_distributions,
    ...
    for c in client_list:
        loss, acc, f1, prec, rec = c.evaluate_global_model(model.state_dict(), time_step)
        best_global_results[c.client_id] = dict(
            loss=loss, accuracy=acc, f1=f1, precision=prec, recall=rec
        )
